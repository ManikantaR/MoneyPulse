...
| **Audit logging**   | Same PostgreSQL DB, `audit_logs` table. Captures: login, login_failed, password_changed, role_changed, transaction_edited, budget_exceeded, file_imported, csv_exported, file_uploaded, file_deleted |
| **Encryption at rest** | AES-256-GCM app-layer encryption for PII columns: `last_four`, `original_description`, `ha_webhook_url`, `notification_email`, `input_text`, `output_text`. Key via `ENCRYPTION_KEY` env var (64-char hex). Migration script for existing data |
| **Security headers** | `helmet` middleware on NestJS API. CSP + X-Frame-Options + Referrer-Policy + Permissions-Policy on Next.js frontend middleware                              |
| **JWT hardening**    | Algorithm pinned to HS256 in both sign and verify. Configurable expiry via `JWT_EXPIRES_IN` env var                                                        |
| **Cookie security**  | All cookies (access_token, refresh_token, device_id) are httpOnly + secure + sameSite:lax                                                                  |
| **Network isolation**| PostgreSQL, Redis, PDF parser bound to 127.0.0.1 in Docker Compose. Redis requires password authentication                                                |
| **Swagger**          | Gated behind `NODE_ENV !== 'production'`. Not exposed in production builds                                                                                 |
| **CSV injection**    | Formula chars (`=+@-`) stripped from CSV export fields to prevent spreadsheet formula injection                                                             |
| **PII sanitization** | SSN, credit card, email, phone, DOB, account number, street address patterns detected and masked in AI prompts (both Python and TypeScript)                 |
| **AI observability** | Admin-only AI logs dashboard: prompt history, PII detection alerts, model performance stats. Role-guarded with `@Roles('admin')`                           |
